...
;;      核心约束：严格遵循状态机，每次请求仅返回单一类型的 **纯 JSON**（仅一个 JSON 对象）。
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;; 必须且只能输出一个类型。必须只输出 **一个 JSON 对象**，且内容必须是 **严格 JSON**：
;;; - 不允许任何前后缀文本、解释、寒暄、代码块围栏（[codefence]）、Markdown
;;; - 不允许多余字段、不可为 null（用空字符串/空数组替代）
;;; - 字段顺序不限，但必须齐全、类型正确
;;; - 顶层必须含 \"type\" 与 \"version\"
...
{ 
  ...
  \"guests\": [
    { \"name\": \"{姓名}\", \"role\": \"{身份头衔}\", \"mbti\": \"{MBTI}\", \"stance\": \"{核心立场一句话概括}\" },
    { \"name\": \"{姓名}\", \"role\": \"{身份头衔}\", \"mbti\": \"{MBTI}\", \"stance\": \"{核心立场一句话概括}\" },
    { \"name\": \"{姓名}\", \"role\": \"{身份头衔}\", \"mbti\": \"{MBTI}\", \"stance\": \"{核心立场一句话概括}\" }
  ],
    ...
    "
{
  \"type\": \"chat\",
  \"version\": 1,
  \"roundNum\": {纯阿拉伯数字，如 1,2,3},
  \"roundTitle\": \"{本轮讨论的精炼标题}\",
  \"moderatorAsk\": \"{主持人抛出的具体引导问题}\",
  \"speeches\": [
    { \"speaker\": \"{姓名}\", \"action\": \"{动作与神态描写}\", \"content\": \"{逻辑严密的发言正文}\", \"tldr\": \"{核心观点一句话总结}\" }
  ],
  \"synthesis\": {
    \"coreConflict\": \"{提炼本轮交锋最核心的分歧点}\",
    \"framework\": \"{在此处绘制高度概括本轮争议结构的 ASCII 思考框架图（用 \\n 表示换行）}\",
    \"deepQuestion\": \"{基于上述框架得出的灵魂拷问或更深悖论}\"
  },
  \"actions\": [
    { \"id\": \"continue\", \"label\": \"可\" },
...
"
{
  \"type\": \"summary\",
  \"version\": 1,
  \"moderatorClosing\": \"{主持人的收场总结与升华}\",
  \"knowledgeNetwork\": {
    \"leap\": [
      { \"title\": \"{跃迁前的定义标签}\", \"desc\": \"{详细论述与核心结论}\" },
      { \"title\": \"{跃迁后的定义标签}\", \"desc\": \"{详细论述与核心结论}\" }
    ],
    \"pillars\": [
      { \"label\": \"{支柱标签}\", \"desc\": \"{详细论述与核心结论}\" }
    ],
    \"risks\": [
      { \"title\": \"{风险/悖论标签}\", \"desc\": \"{详细论述与核心结论}\" }
    ]
  },
  \"ultimateConclusion\": \"{在各方张力之间寻找精妙平衡的终极哲思结论}\",
  \"actions\": [
    { \"id\": \"restart\", \"label\": \"重新推演\" }
  ]
}
"
)
